...
config.framework = 'QBCORE' -- ESX | QBCORE | VRP | STANDALONE (VRP not supported yet, but you can use standalone, it will work!)
config.Mysql = 'oxmysql' -- 'ghmattisql' | 'mysql-async' | 'oxmysql'
config.weight_type = false -- ESX item weight or limit type
config.vehicle_table = 'player_vehicles' -- change this if you use different sql table for player vehicles (note identifier steam is always being used here)
config.Owner_column = 'license' -- owner column name for identifier eg.steam hex,licensed
...
config.commanditem_permission = { -- item command permission -- standalone purpose!
	...
	'license:b4504a77799e08e3bf791db09df677403d021ac5',
}
