...
RegisterKeyMapping(Config.Command['GANGMENU'] or 'gangmenu', 'Gang Menu', 'keyboard', 'f10')
...
function GetPlayersInArea()
    ...
    for _, player in pairs(GetActivePlayers()) do
        if player ~= playerId then
            ...
            if #(playerPos - targetPos) < 5.0 then
                PlayersInArea[player] = player
            end
        end
    end
end
                    ...
                    for _, player in pairs(PlayersInArea) do
                        if NetworkIsPlayerActive(player) then
                            if Config.OtherOptions['anonymousNames'] then
                                WarMenu.Button(GetPlayerServerId(player))
                            else
                                WarMenu.Button(GetPlayerName(player))
                            end

                            if WarMenu.IsItemSelected() then
                                TriggerServerEvent('rcore_gangs:invitePlayer', GetPlayerServerId(player))
            ...
            if WarMenu.Begin('GANG_PLAYERS') then
                if next(PlayersInArea) == nil then
                    ...
                    for _, player in pairs(PlayersInArea) do
                        if NetworkIsPlayerActive(player) then
                            if Config.OtherOptions['anonymousNames'] then
                                if WarMenu.MenuButton(GetPlayerServerId(player), 'GANG_ACTIONS') then
                                    WarMenu.SetSubTitle('GANG_ACTIONS', GetPlayerServerId(player))
                                    selectedPlayer = player
                                end
                            else
                                if WarMenu.MenuButton(GetPlayerName(player), 'GANG_ACTIONS') then
                                    WarMenu.SetSubTitle('GANG_ACTIONS', GetPlayerName(player))
                                    selectedPlayer = player
                                end
                            end
