CreateThread(function()
    ...
    FetchPlayerGangData = function(source)
        local queryString = [[
            SELECT
                ...
                gangs.name,
                gangs.color,
                gangs.presence_capture_disabled AS presenceDisabled,
                gangs.checkpoints,
                gangs.vehicles,
                gangs.identifier = %s.%s AS isLeader,
                gangs.identifier AS myId
            FROM gangs
            INNER JOIN %s ON
                gangs.id = %s.gang_id
            WHERE
    ...
    FetchGangMembers = function(gangId)
        ...
        if Config.Framework and Config.Framework == 1 then
            return DB.fetchAll(string.format(queryString, "CONCAT(firstname, ' ', lastname)", _identifier, _table), {
                ['@gangId'] = gangId
            })
    ...
    FetchGangByKey = function(key, value)
        return DB.fetchAll(string.format([[
            SELECT
                id,
                identifier,
                tag,
                name,
                color,
                presence_capture_disabled AS presenceDisabled
            FROM gangs
            WHERE
                %s = @value
        ]], key), {
    ...
    InsertGangZone = function(gangId, zoneName, loyalty)
        return DB.execute([[
            ...
            (
                name, gang_id, loyalty
            )
            VALUES
            (
                @zoneName, @gangId, @loyalty
            )
        ]], {
            ['@gangId'] = gangId,
            ['@zoneName'] = zoneName,
    ...
    FetchAllRivalries = function()
        return DB.fetchAll([[
            SELECT
                id,
                zone,
                attacking_gang_id AS attackingGangId,
                defending_gang_id AS defendingGangId,
    ...
    FetchUnclaimedRivalries = function()
        return DB.fetchAll([[
            SELECT
                ...
                zone,
                funds,
                IF(defender_sold > attacker_sold, defending_gang_id, attacking_gang_id) AS gangId
            FROM rivalries
            WHERE
                ends_at <= NOW()
            AND
    ...
    FetchFinishedRivalry = function(rivalryId, gangId)
        return DB.fetchAll([[
            ...
            AND
                id = @rivalryId
            AND
                ((attacking_gang_id = @gangId AND attacker_sold >= defender_sold) OR (defending_gang_id = @gangId AND attacker_sold < defender_sold))
        ]], {
            ['@rivalryId'] = rivalryId,
            ['@gangId'] = gangId
        })[1]
    ...
    FetchRivalryByGangAndZone = function(gangId, zoneName)
        return DB.fetchAll([[
            ...
            AND
                zone = @zoneName
            AND
                (attacking_gang_id = @gangId OR defending_gang_id = @gangId)
        ]], {
            ['@gangId'] = gangId,
            ['@zoneName'] = zoneName
        })[1]
    ...
    InsertRivalry = function(attackingGangId, defendingGangId, zoneName)
        return DB.execute([[
            ...
            (
                zone, attacking_gang_id, defending_gang_id, funds, attacker_sold, defender_sold, ends_at
            )
            VALUES
            (
                @zoneName, @attackingGangId, @defendingGangId, @funds, @attackerSold, @defenderSold, DATE_ADD(NOW(), INTERVAL @duration HOUR)
            )
        ]], {
            ['@zoneName'] = zoneName,
