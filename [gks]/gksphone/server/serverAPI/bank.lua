        ...
        if bidentifier == zPlayer.PlayerData.citizenid then
            ...
            if balance < tonumber(c) then
                ...
                MySQL.Async.execute('DELETE FROM gksphone_vehicle_sales WHERE owner = @owner AND plate = @plate', {
                    ...
                    MySQL.Async.fetchAll('SELECT gksphone_vehicle_sales.*, '..Config.OwnedVehicles..'.vehicle, '..Config.OwnedVehicles..'.hash  FROM gksphone_vehicle_sales, '..Config.OwnedVehicles..' WHERE gksphone_vehicle_sales.plate = '..Config.OwnedVehicles..'.plate AND gksphone_vehicle_sales.owner = '..Config.OwnedVehicles..'.citizenid AND '..Config.OwnedVehicles..'.carseller = 1 ORDER BY TIME DESC LIMIT 30', {}, function(result)
                        local valcik = {}
                        for i = 1, #result, 1 do
                            local name = ""
