...
var BG = {
	...
	openingPage: function(bgColor)
	{
		...
		setTimeout(function()
		{
			...
			document.querySelectorAll('.item-page').forEach((elem) => {
	...
	setConfig: function()
	{
		...
		CONFIG.rulesContent.forEach((element) => 
		{
			warpRules.innerHTML += `<div class="item-rules"><div class="rules-tittle">${element.title}</div>` + BG.resultText(element.text); +`</div>`;
		});
...
var V = {
	...
	onClickMute: function(data)
	{
		...
		else
		{
			...
			audioblock.volume = 0;
			V.isMuted = true;
			data.src = "img/icons/volume-mute.png";
		}
	},
	...
	updateIcon: function()
	{
		if (CONFIG.volume <= 0)
		{
			iconVolume.src = "./img/icons/volume-mute.png";
		}
		else if(CONFIG.volume < 0.5)
		{
			iconVolume.src = "./img/icons/volume-low.png";
		}
		else
		{
			iconVolume.src = "./img/icons/volume-loud.png";
		}
	},
...
var C = {
	...
	animation: function()
	{
		...
		{
			C.circles[i].update();
			if (C.circles[i].alpha < 0 || C.circles[i].r < 2)
			{
				C.circles.splice(i, 1);
			}				
